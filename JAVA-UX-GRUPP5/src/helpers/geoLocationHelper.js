...
let getMinMaxValCoord = async (param, level, boundary, downsample) => {
  let validTimeData = await smhiService.fetchData(
    'https://opendata-download-metanalys.smhi.se/api/category/mesan1g/version/2/validtime.json',
  )
  ...
  data.geometry.coordinates.forEach((c, i) => {
    ...
    let isSweden = rayCastingAlgorithm(
      latitude,
      longitude,
      cornersLat,
      cornersLong,
